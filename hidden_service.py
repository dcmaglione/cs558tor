...
def set_up_intro_point(base_circuit, introduction_point_router, rendezvous_point_router, hidden_service, rendezvous_cookie):
    def cb(response, intro_circuit):
        ...
        # The original circuit of which to add the rendezvous point (3 nodes now, will soon be 4)
        original_circuit = base_circuit
        # Introduction point (approved by the hidden service)
        introduction_point = intro_circuit.last_node
        ...
        introduction_point = CircuitNode(
            introduction_point.router, key_agreement_cls=TapKeyAgreement)
        ...
        cell_acknowledgement = send_relay_cell(
            intro_circuit, introduce_cell, CellRelayIntroduceAck)
        ...
        # Derivative key data (as in Sections 5.1.3 and 5.2)
        auth = handshake_response[128:]
        # Client's public key, g^y = Y, in bytes
        Y_bytes = handshake_response[:128]

        HASH_LEN = 20  # SHA1 digest length
        # Should be HASH_LEN bytes according to the spec.
        if len(auth) != HASH_LEN:
            raise KeyAgreementError('Received wrong length SHA1 digest.')

        Y = dh_public_from_bytes(Y_bytes)  # Y = g^y

        shared_secret = raise_exponent(Y, x)  # g^xy
        ...
        # Cut unused bytes
        shared_secret_key = key_material[:KeyAgreement.KEY_MATERIAL_LENGTH]
...
def extend_to_hidden(circuit, hidden_service):
    ...
    establish_cell = CellRelayEstablishRendezvous(
        rendezvous_cookie,
        circuit.id
    )
    cell_established = send_relay_cell(
        circuit, establish_cell, CellRelayRendezvousEstablished)
    ...
    introduction_router_generator = get_introduction_routers(
    ...
    rendezvous_point_router = random.choice(
        convenient_rendezvous_points).router

    # Set up an introduction point for the hidden service
    introduction_point = set_up_intro_point(
        circuit, introduction_point_router, rendezvous_point_router, hidden_service, rendezvous_cookie)
...
def get(hs_name, port=80, path="", live=False):
    ...
    with tor.create_circuit(3) as circuit:
        ...
        hidden_service, address = stream_prepare_address(
            tor_stream, hs_name, port)
        ...
        logger.info('Extending #%x circuit for hidden service %s',
                    circuit.id, hidden_service.hostname)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--live', action='store_true',
                        help='connect to the real Tor network')
